...
without covariates, in both the refit the model, and the resample the MLEs case.
...
# bootstrap resamples we would like to use (R). The transform = T argument
# can be used to produce bootstraps on the parameter-scale, rather than 
# the link-scale:
...
The transform_output function within the GAI package uses model settings stored
