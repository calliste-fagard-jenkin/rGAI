...
# install_github("calliste-fagard-jenkin/GAI", quiet = F, ref = "e50ffe2")
# install()
  library(GAI)
  ...
  # always be provided if running models on a large server, to avoid using all
  # of its resources:
  ...
  # In the case where a model contains no covariate values, a blank data.frame
  # can be used instead:
  transform_output(my_mixture_GAI, data.frame())
