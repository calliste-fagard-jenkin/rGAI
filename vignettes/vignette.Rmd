...
n-component mixture model. The choice of seasonal flight pattern can be specified
using the options "stopover", "spline" and "mixture", for each of the above
...
include covoariates on the values of the means, disperisions and weights for their normal distribution components.
...
\textcolor{red}{reminder to myself to add in a check for non-time-variance or 
NA values in covariate formulas}
...
lines(rep(4.1, 2), c(0.005, 0.032), col = plot.col)
lines(rep(13.0, 2), c(0.005, 0.16), col = plot.col)
lines(rep(23.0, 2), c(0.005, 0.054), col = plot.col) 

# Mean brood dispersion lines:
lines(c(1, 8), rep(0.005, 2), col = plot.col)
lines(c(9, 18), rep(0.005, 2), col = plot.col)
lines(c(19, 26), rep(0.005, 2), col = plot.col)

# The length of the three horizontal bars that measure the width of our broods:
brood_widths <- c(7, 9, 7)
...
w_guesses <- c(0.032 - 0.005, 0.16 - 0.005, 0.054 - 0.005) %>% sum_to_one
```


Let's consider our brood_specific_fit once more, but using the 
transform_starting_values function to obtain better starting values, without knowing the distribution our data came from. The transform_starting_values 
...
# distributional parameter by adding in a dist.par entry to this list.
...
# bootstrap resamples we would like to use:
...
parameter values, the estimated site totals (site superpopulations) for each 
site, and the average estimated site totals. The $1 - \alpha$ confidence intervals are available in the \$par, \$sites and \$index elements of the
bootstrap output for the parameters, individual site totals and average site 
total, respectively:
...
# Model Outputs
\textcolor{red}{A section to be added explaining all of the objects the fit_GAI
functions adds to its output, and how to use these to make plots, calculate
the 'actual' Generalised Abundance Index (which I can easily add as an output
itself), etc.}
...
\textcolor{red}{I'll explain how to backtransform parameter values here once
a more user friendly version of the function is written}
